...
function ClockInInventoryModal({
    open,
    onClose,
    storeName,
}: {
                ...
                <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white shadow-lg">
                    ...
                    <div className="flex items-center justify-between border-b border-gray-200 px-4 py-3">
                        <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm font-semibold text-gray-900">Inventory Update Required</p>
                        </div>

                        {/* optional X (still forces the same action because only one button exists) */}
                        <button
                            type="button"
                            onClick={onClose}
                    ...
                    <div className="px-4 py-4">
                        <p className="text-sm text-gray-700">
                            You are clocked in{storeName ? ` at ${storeName}` : ""}.
                        </p>

                        <div className="mt-3 rounded-lg border border-yellow-200 bg-yellow-50 px-3 py-2 text-sm text-gray-800">
                            Please update your inventory now and click <span className="font-semibold">Save / Submit</span>.
                        </div>
...
export default function InventoryPageClient() {
    ...
    // OPTIONAL: if you already have store in context, use it
    // const { store } = useEmployee();
    const storeName = undefined;

    const [open, setOpen] = useState(false);

    // Open modal when notice is present
    useEffect(() => {
        if (notice === "clockin") { setOpen(true); }
    }, [notice]);

    // Remove notice param after closing so it doesn't reopen on refresh
    const closeAndClearNotice = () => {
        setOpen(false);
    ...
    return (
        ...
        <main className="w-full min-h-screen px-4 sm:px-8 md:px-12 lg:px-16 pb-8 bg-gray-50 dark:bg-gray-900">
            <div className="p-4">
                <ClockInInventoryModal
                    open={open}
                    onClose={closeAndClearNotice}
                    storeName={storeName}
                />
            </div>
            {/* ✅ TRUE sticky header */}
