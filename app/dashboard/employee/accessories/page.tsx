...
import InventoryGroupedTable from "@/components/employee/accessories/InventoryGroupedTable";
import { fetchInventory, InventoryGrouped, InventoryItem, StoreInventoryResponse, updateInventory } from "@/services/accessories/accessoriesService";
import { AccessoryFormValues, accessorySchema } from "@/schemas/accessoryFormSchema";
import AccessorySearch from "@/components/employee/accessories/AccessorySearch";
...
export default function InventoryPage() {
                ...
                resp.storeInventory.forEach((group) => {
                    group.inStock.forEach((i) =>
                        flattened.push({
                            ...
                            caseQuantity: i.caseQuantity ?? 0,
                            glassQuantity: i.glassQuantity ?? 0,
                        })
                    );
    ...
    const onCaseQuantityChange = (id: number, newQty: number, regroup = false) => {
        const idx = idToIndex.get(Number(id));
        if (idx !== undefined) {
            setValue(`products.${idx}.caseQuantity`, newQty, { shouldDirty: true });
        }

        setGroupedInventory((prev) =>
            prev.map((g) => {
                const updatedInStock = g.inStock.map((x) =>
                    Number(x.id) === id ? { ...x, caseQuantity: newQty } : x
                );
                ...
                if (regroup) {
                    ...
                    return {
                        ...g,
                        inStock: all.filter((p) => p.caseQuantity > 0),
                        outofStock: all.filter((p) => p.caseQuantity === 0),
                    };
                }
    ...
    // ✅ Updated quantity change logic (no live re-group)
    const onGlassQuantityChange = (id: number, newQty: number, regroup = false) => {
        const idx = idToIndex.get(Number(id));
        if (idx !== undefined) {
            setValue(`products.${idx}.glassQuantity`, newQty, { shouldDirty: true });
        }

        setGroupedInventory((prev) =>
            prev.map((g) => {
                const updatedInStock = g.inStock.map((x) =>
                    Number(x.id) === id ? { ...x, glassQuantity: newQty } : x
                );
                const updatedOutStock = g.outofStock.map((x) =>
                    Number(x.id) === id ? { ...x, glassQuantity: newQty } : x
                );
                ...
                if (regroup) {
                    ...
                    return {
                        ...g,
                        inStock: all.filter((p) => p.glassQuantity > 0),
                        outofStock: all.filter((p) => p.glassQuantity === 0),
                    };
                }
            ...
            <section className="mb-8 rounded-3xl border border-sky-200 dark:border-sky-900/60 bg-sky-50/60 dark:bg-sky-950/30 shadow-sm backdrop-blur-sm">
                <div className="px-4 sm:px-6 py-4 flex items-center gap-2 border-b border-sky-200/70 dark:border-sky-800/60">
                    ...
                    <h2 className="text-sm font-semibold text-sky-900 dark:text-sky-200">
                        Find Accessories across stores
                    </h2>
                </div>

                <div className="px-4 sm:px-6 py-4">
                    {data && data.length > 0 && <AccessorySearch data={data} />}
                </div>
            </section>
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <InventoryGroupedTable
                            ...
                            getValues={getValues}
                            setValue={setValue}
                            onCaseQuantityChange={onCaseQuantityChange}
                            onGlassQuantityChange={onGlassQuantityChange}
